      ...
      attrList = v: (lib.attrsets.mapAttrsToList
        (key: value:
          # do not include empty attrs
            if value != {}
            then "${key} ${toZellij {depth = depth + 1;} value}"
            else key)
        v);
    ...
    in
      ...
      then let
        ...
        inner =
          ...
          else if hasSimpleKey cleanAttrs
          then "{ ${builtins.head (attrList cleanAttrs)}; }"
          else "{\n${attrsString}" + "\n" + (tabstr (depth - 1) "}");
      in
  ...
  mkKeybinds = v:
    ...
    (name: value:
      builtins.listToAttrs (map
        (kb: let
          ...
          action = builtins.listToAttrs (map
            ...
            in {
              name =
                if (value == {})
                then builtins.toString key
                else if !builtins.isString value
                then "${key} ${builtins.toString value}"
        ...
        value))
    (builtins.removeAttrs v ["_properties"])
    // {_properties = v._properties or {};};
  ...
  in {
    ...
    keybinds = let
      ...
      swToModesBinds = modes: let
        ...
        in
          if (builtins.typeOf modes == "list")
          then builtins.elem newMode modes
          else modes == newMode;
      in
        lib.filter filter [
          {
            action = [swToModes.normal];
            key = [{Ctrl = "g";} "Esc"];
          }
          {
            action = [swToModes.locked];
            key = [{Ctrl = "g";}];
          }
          {
            action = [swToModes.pane];
            key = [{Ctrl = "p";}];
          }
          {
            action = [swToModes.move];
            key = [{Ctrl = "h";}];
          }
          {
            action = [swToModes.scroll];
            key = [{Ctrl = "s";}];
          }
          {
            action = [swToModes.tab];
            key = [{Ctrl = "t";}];
          }
          {
            action = [swToModes.tmux];
            key = [{Ctrl = "a";}];
          }
          {
            action = [swToModes.resize];
            key = [{Ctrl = "n";}];
          }
          {
            action = [swToModes.session];
            key = [{Ctrl = "o";}];
          }
          {
            action = [swToModes.renametab];
            key = [","];
          }
          {
            action = [swToModes.renamepane];
            key = ["$"];
          }
        ];
      ...
      config = {
        ...
        resize =
          swToModesBinds (with modes; [normal move scroll session tab pane])
          ++ [
            {
              action = [{Resize = "Increase";}];
              key = ["="];
        ...
        pane =
          swToModesBinds (with modes; [normal move scroll session tab resize renamepane])
          ++ [
            {
              action = ["SwitchFocus"];
              key = ["p"];
            ...
            {
              action = [{NewPane = "Left";} swToModes.normal];
              key = ["n"];
            }
            {
              action = [{NewPane = "Down";} swToModes.normal];
              key = ["d"];
            }
            {
              action = [{NewPane = "Right";} swToModes.normal];
              key = ["r"];
            }
            {
              action = ["CloseFocus" swToModes.normal];
              key = ["x"];
            }
            {
              action = ["ToggleFocusFullscreen" swToModes.normal];
              key = ["f"];
            }
            {
              action = ["TogglePaneFrames" swToModes.normal];
              key = ["z"];
            }
            {
              action = ["ToggleFloatingPanes" swToModes.normal];
              key = ["w"];
            }
            {
              action = ["TogglePaneEmbedOrFloating" swToModes.normal];
              key = ["e"];
            }
          ]
          ++ defaultBinds
        ...
        move =
          swToModesBinds (with modes; [normal pane resize scroll session tab])
          ++ [
            {
        ...
        tab =
          swToModesBinds (with modes; [normal move pane resize scroll session renametab])
          ++ [
            {
            ...
            {
              action = ["CloseTab" swToModes.normal];
              key = ["x"];
            }
            {
              action = ["ToggleActiveSyncTab" swToModes.normal];
              key = ["s"];
            }

            {
              action = [{GoToTab = 1;} swToModes.normal];
              key = ["1"];
            }
            {
              action = [{GoToTab = 2;} swToModes.normal];
              key = ["2"];
            }
            {
              action = [{GoToTab = 3;} swToModes.normal];
              key = ["3"];
            }
            {
              action = [{GoToTab = 4;} swToModes.normal];
              key = ["4"];
            }
            {
              action = [{GoToTab = 5;} swToModes.normal];
              key = ["5"];
            }
            {
              action = [{GoToTab = 6;} swToModes.normal];
              key = ["6"];
            }
            {
              action = [{GoToTab = 7;} swToModes.normal];
              key = ["7"];
            }
            {
              action = [{GoToTab = 8;} swToModes.normal];
              key = ["8"];
            }
            {
              action = [{GoToTab = 9;} swToModes.normal];
              key = ["9"];
            }
            {
              action = [{NewTab = {};} swToModes.normal];
              key = ["n"];
            }
          ]
          ++ defaultBinds
        ...
        scroll =
          swToModesBinds (with modes; [normal move pane resize session tab])
          ++ [
            {
              action = ["EditScrollback" swToModes.normal];
              key = ["e"];
            }
            {
              action = ["ScrollToBottom" swToModes.normal];
              key = [{Ctrl = "c";}];
            }
            {
              action = ["ScrollDown"];
            ...
            }
            {
              ...
              key = [{Ctrl = "f";} "PageDown" "Right" right];
            }
            {
              action = ["PageScrollUp"];
              key = [{Ctrl = "b";} "PageUp" "Left" left];
            }
            {
        ...
        renametab =
          swToModesBinds (with modes; [normal])
          ++ [
            {
              action = [{TabNameInput = 27;} swToModes.tab];
              key = ["Esc"];
            }
          ]
          ++ defaultBinds
        ...
        renamepane =
          ...
          ++ [
            {
              action = [{PaneNameInput = 27;} swToModes.pane];
              key = ["Esc"];
            }
          ]
          ++ defaultBinds
        ...
        session =
          swToModesBinds (with modes; [normal move pane resize scroll tab])
          ++ [
            {
              action = ["Detach"];
              key = ["d"];
        ...
        tmux =
          swToModesBinds (with modes; [normal renamepane renametab])
          ++ [
            {
              action = [{NewPane = "Down";} swToModes.normal];
              key = ["-"];
            ...
            {
              action = [{Write = 1;} swToModes.normal];
              key = [{Ctrl = "a";}];
            }
            ...
            {
              action = ["GoToPreviousTab" swToModes.normal];
              key = [{Ctrl = "y";}];
            }
            {
              action = ["GoToNextTab" swToModes.normal];
              key = [{Ctrl = "o";}];
            }
